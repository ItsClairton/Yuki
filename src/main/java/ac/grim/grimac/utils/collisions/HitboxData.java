...
public enum HitboxData {
    ...
    SNOW((player, item, version, data, x, y, z) ->
            new SimpleCollisionBox(0, 0, 0, 1, data.getLayers() * 0.125, 1),
            StateTypes.SNOW),
    ...
    WALL_HANGING_SIGNS((player, item, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    WALL_SIGN((player, item, version, data, x, y, z) -> switch (data.getFacing()) {
    ...
    WALL_HANGING_SIGN((player, item, version, data, x, y, z) -> switch (data.getFacing()) {
        case NORTH, SOUTH -> new ComplexCollisionBox(new HexCollisionBox(0.0D, 14.0D, 6.0D, 16.0D, 16.0D, 10.0D),
                new HexCollisionBox(1.0D, 0.0D, 7.0D, 15.0D, 10.0D, 9.0D));
