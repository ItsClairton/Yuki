...
public class GrimLog extends BaseCommand {
    ...
    public void onLog(CommandSender sender, int flagId) {
        ...
        if (builder == null) {
            String failure = Yuki.getInstance().getConfigManager().getConfig().getStringElse("upload-log-not-found", "%prefix% &cUnable to find that log");
            sender.sendMessage(MessageUtil.format(failure));
        } else {
            String uploading = Yuki.getInstance().getConfigManager().getConfig().getStringElse("upload-log-start", "%prefix% &fUploading log... please wait");
            String success = Yuki.getInstance().getConfigManager().getConfig().getStringElse("upload-log", "%prefix% &fUploaded debug to: %url%");
            String failure = Yuki.getInstance().getConfigManager().getConfig().getStringElse("upload-log-upload-failure", "%prefix% &cSomething went wrong while uploading this log, see console for more information.");

            sender.sendMessage(MessageUtil.format(uploading));

            FoliaScheduler.getAsyncScheduler().runNow(Yuki.getInstance().getPlugin(), (dummy) -> {
                try {
                    URL mUrl = new URL("https://paste.grim.ac/data/post");
                    HttpURLConnection urlConn = (HttpURLConnection) mUrl.openConnection();
                    urlConn.setDoOutput(true);
                    urlConn.setRequestMethod("POST");
                    urlConn.addRequestProperty("User-Agent", "GrimAC/" + Yuki.getInstance().getExternalAPI().getGrimVersion());
